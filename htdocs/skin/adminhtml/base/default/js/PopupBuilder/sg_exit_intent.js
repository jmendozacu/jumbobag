...
SGExitIntnetPopup.prototype.softMode = function(id) {
	var that = this;
	$jsg(document).mouseleave(function() {
		var result = that.canOpen(id, 'soft');
	 	if(result){
			return;
		}
		if ($jsg("#sgcolorbox").css("display") !== "block") { /* Check colorbox is open */
...
SGExitIntnetPopup.prototype.canOpen = function(id, type) {
	...
	if(!$jsg.cookie('SGExitIntnetPopup'+id+type)) {
...
SGExitIntnetPopup.prototype.setCookies = function(id, type) {
	...
	$jsg(document).ready(function() {
		$jsg('#sgcolorbox').on('sgPopupClose', function(e) {
			if(that.getExpireTime() !== 'always') {
			$jsg.cookie('SGExitIntnetPopup'+id+type,id, { expires: date});
			if(that.getExpireTime() == 'perSesion') {
				$jsg.cookie('SGExitIntnetPopup'+id,id);
			}
		}
		});
	});
