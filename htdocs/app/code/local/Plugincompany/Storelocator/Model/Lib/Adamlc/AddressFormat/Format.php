 ...
 */
class Plugincompany_Storelocator_Model_Lib_Adamlc_AddressFormat_Format implements \ArrayAccess
{
    protected $locale;
     ...
     *
     * @var mixed Plugincompany_Storelocator_Model_Lib_Adamlc_AddressFormat_Format
     * @access private
     */
    ...
    public function setLocale($locale)
    {
        ...
        $file = Mage::getBaseDir() . DS . 'app' . DS . 'code' . DS . 'local' . DS . 'Plugincompany' . DS . 'Storelocator' . DS . 'Model' .DS . 'Lib' . DS . 'Adamlc' . DS . 'AddressFormat' . DS . 'i18n' .DS . strtoupper($locale) . '.json';
        if (file_exists($file)) {
            ...
            if (is_array($meta)) {
                ...
                throw new Exception('Locale "' . $locale . '" could not be parsed correctly');
            }
        } else {
            throw new Exception('Locale not supported by this library');
        }
    }
    ...
    public function setAttribute($attribute, $value)
    {
        ...
        if (isset($this->input_map[$attribute])) {
            ...
            throw new Exception('Attribute not supported by this library');
        }
    }
    ...
    public function validAddressPieces()
    {
        ...
        if (isset($this->locale['fmt']))
        {
            ...
            throw new Exception('Locale missing format');
        }
    }
