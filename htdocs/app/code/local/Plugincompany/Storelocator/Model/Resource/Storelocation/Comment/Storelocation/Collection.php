 ...
 */
/**
 * Store location comments resource collection model
 *
 * @category    Plugincompany
 * @package     Plugincompany_Storelocator
 * @author      Milan Simek
 */
class Plugincompany_Storelocator_Model_Resource_Storelocation_Comment_Storelocation_Collection
    extends Plugincompany_Storelocator_Model_Resource_Storelocation_Collection {
    /**
     * Entities alias
     *
     * @var array
    ...
    protected function _construct() {
        ...
        $this->_commentStoreTable = Mage::getSingleton('core/resource')->getTableName('plugincompany_storelocator/storelocation_comment_store');
    }
    /**
     * init select
     * @access protected
    ...
    public function addEntityFilter($entityId) {
        $this->getSelect()->where('ct.storelocation_id = ?', $entityId);
        return $this;
    }
    ...
    protected function _joinFields() {
        $commentTable = Mage::getSingleton('core/resource')->getTableName('plugincompany_storelocator/storelocation_comment');
        $this->getSelect()
            ->join(array('ct' => $commentTable),
                'ct.storelocation_id = main_table.entity_id',
                array(
                    'ct_title'=>'title',
                    'rating'=>'rating',
    ...
    protected function _applyStoresFilterToSelect(Zend_Db_Select $select = null) {
        ...
        if (is_array($storesIds) && !empty($storesIds)) {
            ...
            if (version_compare(Mage::getVersion(), '1.6', '>=')) {
                ...
                $sIds = implode(',', $storesIds);
               $inCond = "store.store_id IN({$sIds})";
            }
