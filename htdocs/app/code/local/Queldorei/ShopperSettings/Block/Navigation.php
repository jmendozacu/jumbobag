...
class Queldorei_ShopperSettings_Block_Navigation extends Mage_Catalog_Block_Navigation
{
	                                               ...
	                                               $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
	{
		...
		$image_url = $cur_category->getData('thumbnail');
		$image_name= $cur_category->getData('name');

		if (!empty($image_url))
		{
			$image_html = '<img class="category_image" width="150" src="'.Mage::getBaseUrl('media').'catalog/category/'.$image_url.'" alt="'.$image_name.'">';
		...
		$html[] = '<span>' . $this->escapeHtml($category->getName()).' '.$image_html.'</span>';
		$html[] = '</a>';
