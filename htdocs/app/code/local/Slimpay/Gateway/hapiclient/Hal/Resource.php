...
final class Resource {
	...
	public static function fromJson($json) {
		...
		return new Resource(
			...
			self::extractLinks($json),
			self::extractEmbeddedResources($json)
		);
	}
	...
	private static function extractLinks(array $json) {
		$links = [];
		...
		$_links = $json['_links'];
		foreach ($_links as $name => $uniqueOrArray) {
			if (isset($uniqueOrArray[0])) { // Array of Links
		    	$aArrayOfLinks = [];
		    	foreach ($uniqueOrArray as $link)
		    		$aArrayOfLinks[] = Link::fromJson($link);

		    	$links[$name] = $aArrayOfLinks;
			} else // Unique Link
		    	$links[$name] = Link::fromJson($uniqueOrArray);
		}

		return $links;
	}

	private static function extractEmbeddedResources(array $json) {
		$embeddedResources = [];

		if (!isset($json['_embedded']))
			return $embeddedResources;

		// Iterate through embedded Resources
		$_embedded = $json['_embedded'];
		foreach ($_embedded as $name => $uniqueOrArray) {
			if (isset($uniqueOrArray[0])) { // Array of embedded Resources
		    	$aArrayOfLinks = [];
		    	foreach ($uniqueOrArray as $resource)
		    		$aArrayOfLinks[] = Resource::fromJson($resource);

		    	$embeddedResources[$name] = $aArrayOfLinks;
			} else // Unique embedded Resource
		    	$embeddedResources[$name] = Resource::fromJson($uniqueOrArray);
		}
