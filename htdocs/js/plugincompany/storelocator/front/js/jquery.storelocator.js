    ...
    $.extend(Plugin.prototype, {
        ...
        autoGeocodeQuery: function (position) {
            ...
            r.geocode({'latLng': latlng}, function (data) {
                if (data !== null) {
                    var originAddress = data.address;
                    jQuery('#' + _this.settings.addressID).val(originAddress);
        ...
        getAddressByMarker: function(markerID) {
            ...
            this.writeDebug('address is', formattedAddress);
        ...
        directionsRequest: function(origin, locID, map) {
            ...
            //hier
            var destinationLatLng = new google.maps.LatLng(locationset[locID].lat,locationset[locID].lng );
            ...
            if(destination) {
                ...
                var request = {
                    //hier
                    origin: origin,
                    destination: /*destination*/ destinationLatLng,
                ...
                $('.' + this.settings.locationList).prepend('<div class="bh-sl-close-directions-container">' +
                    '<a class="print-directions"><i class="fa fa-print" aria-hidden="true"></i></a>' +
        ...
        _existingSelectedFilters: function(key) {
            jQuery('#' + this.settings.taxonomyFilters[key] + ' select').each(function () {
                ...
                for(var k in filters) {
                    if (filters.hasOwnProperty(k) && filters[k].length > 0) {
                        ...
                        for (var z = 0; z < filters[k].length; z++) {
                            ...
                            if(typeof filters[k][z] == "object" && filters[k][z] != null){
                                jQuery.each(filters[k][z],function(key,value){
                                    if(value != null){
                                        taxFilters[k][cnt] = '(?=.*\\b' + value.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1") + '\\b)';
                                        cnt++;
                                    }
                                })
                            }
                            else if(filters[k][z] != null){
                                if(k == _this.settings.nameAttribute){
                                    taxFilters[k][cnt] = filters[k][z].replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1");
                                }else{
                                    taxFilters[k][cnt] = '(?=.*\\b' + filters[k][z].replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1") + '\\b)';
                                }
                                cnt++;
                            }
                        }
