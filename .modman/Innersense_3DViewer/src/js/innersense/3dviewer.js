...
Innersense.Viewer3D = (function(options) {
    ...
    var initializeOnLoad = typeof options.initializeOnLoad === 'undefined' ? false : options.initializeOnLoad
    ...
    var selectOption = function selectOption (optionId, optionValue, optionType) {
        ...
        if (!currentOptions[optionId] || currentOptions[optionId].value !== optionValue) {
            currentOptions[optionId] = createNewOption(optionId, optionValue, optionType)
            updateQueuedOptions()
        }
    }
    ...
    var attachDebugger = function attachDebugger(viewer) {
        ...
        var toggleDebuggerButton = makeButton('Afficher / Cacher le débogueur', 'viewer-3d__toggle-debugger')
        ...
        viewerDebugger.parentNode.insertBefore(toggleDebuggerButton, viewerDebugger.nextSibling)
    ...
    var onViewerMessageReceived = function onViewerMessageReceived(event) {
        ...
        switch (action.type) {
            ...
            case 'badRequest':
                ...
                isWaitingResponse = false
                if (options.debug && isWaitingReturnTimeout) clearTimeout(isWaitingReturnTimeout)
        }
    }
