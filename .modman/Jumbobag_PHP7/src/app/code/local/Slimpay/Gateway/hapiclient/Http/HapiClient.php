...
use HapiClient\Hal\HalResource;
...
final class HapiClient {
	 ...
	 * @param $entryPointUrl	string	The URL to the entry point HalResource.
	...
	/**
	 * @return	string	The URL to the entry point HalResource.
	 */
	 ...
	 *
	 * @return HalResource	The HalResource object returned by the server.
	 */
	public function sendRequest(Request $request) {
		...
		if ($statusCode >= 200 && $statusCode < 300)
			return HalResource::fromJson((string) $httpResponse->getBody());
	 ...
	 * @param $resource	null|HalResource	The resource containing the link you want to follow.
	 *									If null, the entry point HalResource will be used.
	 *
	 * @return HalResource	The HalResource object contained in the last response.
	 */
	public function sendFollow($follow, HalResource $resource = null) {
		if (!$resource)
	 ...
	 * and returns its HalResource object.
	 *
	 * The entry point Resource is only retrieved if needed
	 * and only once per HapiClient instance.
	 * @return	HalResource	The entry point Resource.
	 * @throws HttpException
	 */
	public function getEntryPointResource() {
		static $entryPointResource = null;

		if (!$entryPointResource)
			return $entryPointResource = $this->sendRequest(new Request($this->entryPointUrl));
		else
	 ...
	 * @param $resource	HalResource	The Resource to refresh.
